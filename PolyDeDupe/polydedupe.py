...
def _find_cluster_extremes_shared_semantic(cluster, jaccard_threshold):
    ...
    for element1 in cluster:
        ...
        for element2 in extremes:
            embedding2 = code_embeddings[element2["base_index"]]
            # Use cosine similarity instead of Jaccard similarity
            if cosine_similarity([embedding1], [embedding2])[0][0] >= jaccard_threshold:
                element2["copies"] += 1
                break
        else:
            element1["copies"] = 1
...
def find_extremes_semantic(cluster_list, dataset, jaccard_threshold):
    ...
    Returns:
        extremes_list (List[Dict]):
            Each cluster is reduced to extremes.
            See _find_cluster_extremes_shared_semantic for the definition of extremes.
    """
    global _shared_dataset
    _shared_dataset = dataset
    extremes_list = []
    f = partial(_find_cluster_extremes_shared_semantic, jaccard_threshold=jaccard_threshold)
    with mp.Pool() as pool:
        for extremes in tqdm(
            pool.imap_unordered(
                f,
...
def display_dataset_entries(dataset, duplicate_clusters):
    """
    Fetches and displays the dataset entries for given base_index values in duplicate clusters.
    ...
    Returns:
        None, but prints out the data associated with each base_index in the clusters.
    """
...
def deduplicate_dataset(
    ...
    # duplicate_clusters = filter_cluster(duplicate_clusters, extreme_dict)
